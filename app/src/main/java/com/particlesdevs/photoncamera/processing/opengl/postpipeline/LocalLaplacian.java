...
/**
 * Memory-bounded GPU local-Laplacian filter based on darktable's local
 * contrast module.  It uses darktable's six luminance anchors and remapping
 * curve, but evaluates the remap on the Gaussian levels instead of retaining
 * six complete remapped pyramids.  That keeps the additional storage near one
 * single-channel pyramid, which is important for full-resolution phone RAWs.
 */
public class LocalLaplacian extends Node {
    ...
    @Tunable(title = "Enable", description = "Enable local Laplacian contrast",
            category = "Tone", min = 0, max = 1, defaultValue = 1, step = 1)
    boolean enabled;

    @Tunable(title = "Detail", description = "Increase or remove local contrast",
            category = "Tone", min = -1.0f, max = 4.0f, defaultValue = 0.25f, step = 0.05f)
    float detail;

    @Tunable(title = "Highlights", description = "Local contrast slope in highlights; lower values compress highlights",
            category = "Tone", min = 0.0f, max = 2.0f, defaultValue = 0.5f, step = 0.05f)
    float highlights;
    ...
    @Tunable(title = "Mid-tone Range", description = "Width of the local-contrast region",
            category = "Tone", min = 0.01f, max = 1.0f, defaultValue = 0.5f, step = 0.01f)
    float midtone;
