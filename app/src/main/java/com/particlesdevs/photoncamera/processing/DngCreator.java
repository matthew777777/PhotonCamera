...
public class DngCreator {
    ...
    public void writeBuffer(OutputStream outputStream, ByteBuffer buffer, int width, int height) {
        if (buffer == null) {
            throw new IllegalArgumentException("RAW buffer is null");
        }
        if (!buffer.isDirect()) {
            throw new IllegalArgumentException("DngCreator requires a direct RAW ByteBuffer");
        }
        long expected16 = (long) width * (long) height * 2L;
        Log.d(TAG, "writeBuffer: " + width + "x" + height
                + " rawCapacity=" + buffer.capacity() + " expected16=" + expected16
                + " direct=" + buffer.isDirect());

        ByteBuffer dngData = createDNG(nativePtr, width, height, buffer);
        if (dngData == null) {
            throw new RuntimeException("Failed to create DNG data for " + width + "x" + height);
        }

        // Do not clone the complete DNG into the Java heap. A 25 MP 16-bit mosaic is
        // ~50 MB before TIFF/DNG overhead; duplicating it here can cause ImageSaver OOM.
        // Stream the native direct ByteBuffer in bounded chunks instead.
        try {
            ByteBuffer src = dngData.duplicate();
            src.position(0);
            final int chunkSize = Math.min(1024 * 1024, Math.max(16 * 1024, src.remaining()));
            byte[] chunk = new byte[chunkSize];
            while (src.hasRemaining()) {
                int n = Math.min(src.remaining(), chunk.length);
                src.get(chunk, 0, n);
                outputStream.write(chunk, 0, n);
            }
            outputStream.flush();
            Log.d(TAG, "writeBuffer complete: dngBytes=" + dngData.capacity());
        } catch (Exception e) {
            throw new RuntimeException("Failed to stream DNG data to output stream", e);
        }
    }
