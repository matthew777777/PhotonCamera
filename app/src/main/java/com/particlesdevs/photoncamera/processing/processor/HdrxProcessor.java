...
import com.particlesdevs.photoncamera.processing.opengl.scripts.WronskiMosaicSR;
...
//            }
    ...
    private void ApplyHdrX() {
        ...
        if(images.size() > 1) {
            ...
            // Optional experimental Wronski MFSR path. This deliberately reconstructs
            // a synthetic RGGB CFA at sqrt(2)x linear resolution (about 2x pixels),
            // rather than a full RGB image, so RawTherapee can demosaic it later.
            if (saveRAW >= 1 && "mosaic_sr".equals(PreferenceKeys.getMfsrOutputMode())) {
                WronskiMosaicSR mosaicSR = null;
                try {
                    mosaicSR = new WronskiMosaicSR(new Point(width, height), images);
                    mosaicSR.parameters = processingParameters;
                    // Reuse the exact KernelNet inference already performed inside ESD4D.
                    // The map is RGBA=(s1,s2,rho,1): a learned spatially adaptive
                    // denoising/reconstruction kernel, not a blur/downsampling estimate.
                    mosaicSR.kernelParams = esd4d.kernelsMapCPU;
                    mosaicSR.kernelParamsSize = esd4d.kernelsMapCPUSize;
                    mosaicSR.Run();
                    Point srSize = mosaicSR.getOutputSize();
                    boolean imageSaved = ImageSaver.Util.saveSyntheticMosaicRaw(dngFile, mosaicSR.Output,
                            processingParameters, srSize);
                    if (!imageSaved) {
                        throw new IllegalStateException("Synthetic Mosaic SR DNG writer returned failure for "
                                + srSize.x + "x" + srSize.y);
                    }
                    Log.d(TAG, "Mosaic SR DNG saved: " + srSize.x + "x" + srSize.y
                            + " CFA=" + processingParameters.cfaPattern + " WL=65535 BL=0");
                    processingEventsListener.notifyImageSavedStatus(true, dngFile);
                    // textureBuffer() returns a JVM direct buffer, not Allocator-owned memory.
                    mosaicSR.Output = null;
                } catch (Exception e) {
                    Log.e(TAG, "Mosaic SR reconstruction failed; SR filename will NOT be replaced by a 12 MP file: "
                            + Log.getStackTraceString(e));
                    Path fallbackPath = Paths.get(dngFile.toAbsolutePath().toString().replaceFirst("\\.dng$", "")
                            + "_ESD4D_12MP_FALLBACK.dng");
                    boolean imageSaved = ImageSaver.Util.saveStackedRaw(fallbackPath, output, processingParameters);
                    processingEventsListener.notifyImageSavedStatus(false, dngFile);
                    if (imageSaved) {
                        Log.w(TAG, "Standard ESD4D fallback saved separately: " + fallbackPath);
                    }
                } finally {
                    if (mosaicSR != null) {
                        try { mosaicSR.close(); } catch (Exception ignored) {}
                    }
                }
            }

            if (saveRAW >= 1 && PreferenceKeys.isMfsrKeepBurst()) {
                for (int i = 0; i < images.size(); i++) {
                    Path sourcePath = Paths.get(dngFile.toAbsolutePath().toString().replaceFirst("\\.dng$", "")
                            + String.format(java.util.Locale.ROOT, "_src_%02d.dng", i));
                    ImageSaver.Util.saveStackedRaw(sourcePath, images.get(i).buffer, processingParameters);
                }
            }
        ...
        if ((saveRAW >= 1) && alignAlgorithm != 2) {
            ...
            if (!"mosaic_sr".equals(PreferenceKeys.getMfsrOutputMode())) {
                imageSaved = ImageSaver.Util.saveStackedRaw(dngFile, output, processingParameters);
            }
