...
uniform sampler2D rawPreview;
uniform bool enableRawPreview;
out vec4 Output;
in vec2 texCoord;
void main() {
    vec2 uv = texCoord.xy;
    if(mirror)
        uv.y = 1.0 - uv.y;
    vec4 sourceColor = enableRawPreview ? texture(rawPreview, uv) : texture(sTexture, uv);
    ...
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
            avg += enableRawPreview
                    ? texture(rawPreview, uv + vec2(i*2, j*2) / size)
                    : texture(sTexture, uv + vec2(i*2, j*2) / size);
        }
    }
    avg /= 9.0;
