...
void main() {
    ...
    // Keep the preview compressor bounded. The full capture version derives a
    // histogram curve, which will be added when that stream state is reusable.
    float compression = clamp(compressor, 0.0, 8.0);
    rgb /= 1.0 + rgb * compression * 0.125;
    luma = luminance(rgb);
